...
- 部署验证时发现适配器的刷新成功日志会写入完整 access token；现已改为只记录平台和有效期。
- 刷新失败只记录 HTTP 状态码及响应字段名，不再拼接完整响应正文。
- 后台密码校验的监控记录固定写为 `[REDACTED]`，避免未来调用方式变化后把密码写入 Mongo 日志。
...
- 新镜像上线后容器 health 为 `healthy`，`restart=0`、`OOMKilled=false`；应用直连、Nginx HTTPS 和公网 `https://whumystery.cn/` 均为 200。
- 后台账号校验、错误密码拒绝和正确密码校验均通过，验证过程未输出凭据。
- 2026-09-03 03:20:26—04:20:56 连续取得 13 个五分钟样本。LRobot 从 71.6 MiB 增至 89.43 MiB，后半段增长明显趋缓；全程 health `healthy`、HTTPS 200、重启 0 次、容器和内核 OOM 均为 0。bridge Node 实际 RSS 约 18.8 MiB，失败调用留下的文件页缓存随观察从约 233 MiB 降至约 138 MiB。
- Mongo 二级索引已删除，只保留 `_id_`；总索引占用约 52.3 MiB，日志计数从 2,922,088 继续增长至 2,923,206，说明写入正常且索引没有被应用重建。
- Docker 实际配置已核对为 `json-file`、单文件 10 MB、最多 3 个，LRobot 重启策略为 `unless-stopped`。
- Codex 原刷新令牌已经失效，bridge 在一次 503 后会停止 app-server 并断路，不再每 3 分钟重试。当前正在等待重新完成设备登录。
- 固定测试中，DashScope `deepseek-v3.1` 兜底也返回 403；因此在 Codex 登录恢复前，AI 主持人不可用，经典海龟汤不受影响。该 403 属外部凭据/模型权限问题，未擅自替换用户密钥。

最后一次资源边界日志中 Future 和两个消息队列均为 0，日志队列为 0，所有 dropped/evicted/rejected 指标为 0。两次 Codex 设备登录各等待完整 15 分钟均未收到用户授权，登录进程随后已终止并确认无残留；因此真实 5.3 成功回答仍是需要用户在线交互后补验的一项，不将 503/403 的失败链路误写为通过。
...
- AI 主持人的 Codex 主路径仍需用户完成一次设备登录；DashScope 兜底的 403 也需要持有者核对密钥/模型权限。在两者恢复任一项前，AI 海龟汤不可用，经典海龟汤与网站不受影响。
- 本轮发现并修复了明确的刷新 token 与后台密码落日志问题，但旧代码还有把 `locals()`、BILI 请求参数和大响应写入调试日志的路径。它们已受单条 64 KiB、队列 2,000 条和 Docker 轮转保护，不再能无界占用内存/容器磁盘；隐私层面的全面字段脱敏应单独回归后实施。
- 10 MB × 3 的轮转已经在当前 LRobot 容器生效；Compose 中其他服务也已配置，但现有 Mongo/MySQL/mihomo 等容器要到下次正常重建才会采用。为避免在本轮热重启数据库和消息平台，没有为了改日志驱动而强制重建它们；其中当前最大的 Mongo JSON 日志约 29 MB。
- Mongo `system_log` 数据本身仍会长期增长。二级索引删除解决了约 2.70 GiB 的即时占用，但没有替用户决定历史日志保留期限；后续应先确定取证/审计周期，再设计归档或 TTL。
- 2 GiB 主机仍然偏小，MySQL、Mongo、LRobot、NapCat 与 Codex 同机时会大量使用 swap。本轮边界和自动重启降低了单个组件拖垮整站的概率，但不能替代扩容或拆分数据库/AI 服务。
