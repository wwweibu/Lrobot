...
async def lr5921_dispatch(
    ...
    if msg_type == "forward":
        ...
        url = "send_private_msg" if private_chat else "send_group_msg"
        tag1 = "message"
    tag2, id = ("user_id", user) if private_chat else ("group_id", group)
...
async def lr5921_essence(id, content=None):
    """群聊精华"""
    url = "delete_essence_msg" if content == "删除" else "set_essence_msg"
    data = {"message_id": id}
